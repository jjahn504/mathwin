...
fn main() {
    ...
    let x_min: f64 = -3.0; //정의역(Domain)
    let x_max: f64 = 3.0; //정의역(Domain)
    let y_range: (f64, f64, f64) = my_math.detect_range(y_eq_fx_1, x_min, x_max, width);//치역(Range)
    //보기 좋은 그래프를 위해 5% 여유 공간을 설정함
    let y_min: f64 = y_range.0 * 1.05 - 3.5;
    let y_max: f64 = y_range.1 * 1.05 + 1.0;  
    ...
    //연속 동작을 원하는 코드를 작성합니다.
        ...
        for i in 0..width{
            ...
            let y: f64 = y_eq_fx_1(x);
            
            my_math.circle(x, y, radius_of_point, MathWin::RED);
        }
        for i in 0..width{
            let x: f64 = x_min + (i as f64 * x_delta);  
            let y: f64 = y_eq_fx_2(x);
            
            my_math.circle(x, y, radius_of_point, MathWin::GREEN);
        }
