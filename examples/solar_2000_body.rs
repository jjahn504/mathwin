...
fn main() {
    ...
    let xy_max: f64 = 2.50E13; //2D 상자 크기
    ...
    //GUI:화면 갱신 주기를 초당 N개의 프레임으으로 낮출 수 있습니다.
    ...
    //창을 닫거나 ESC 키를 계속 누르고 있으면 프로그램이 점잖게 종료합니다.(강제 종료: Ctrl+C)  
        ...
        body::Body::calc_new_location(&mut bodies, 1.0E4);   
        ...
        //merge_distance가 너무 작으면 인력의 급격한 증가를 시뮬레이션하기 어려워짐: 역학적 에너지 보존이 안됨
        //근접했을 때의 가속도 (근사)계산이 실제와 너무 달라져서 
        //중력 위치 에너지 감소량에 비해 운동 에너지 증가량이 매우 커짐
        body::Body::merge_if_too_close_more_then_radius(&mut bodies); 
        for body in &mut bodies {
            if body.exist == false{
                continue;
            }          
		...
		string_disp_0 = format!("Many bodies Solars Problem"); 
