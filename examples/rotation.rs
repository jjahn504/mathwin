...
fn main() {
    ...
    //창을 닫거나 ESC 키를 계속 누르고 있으면 프로그램이 점잖게 종료합니다.(강제 종료: Ctrl+C)
        ...
        let mut angle: f64 = pi2;
        
        count = 0;
        while count < 700 {
            //my_math.clear_screen(color_black);
            my_math.line(half_r_max, half_r_max, x_old, y_old, MathWin::BLACK);
            my_math.circle_digit_radius(x_old, y_old, MathWin::BLACK, 10); 
            x_old = x;
            y_old = y;
            ...
            my_math.circle_digit_radius(x, y, MathWin::RED, 10); 
